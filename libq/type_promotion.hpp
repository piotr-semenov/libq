...
template<
    ...
    int delta_e
>
class type_promotion_base<libq::fixed_point<T, _n, _f, _e, op, up>, delta_n, delta_f, delta_e>
{
    ...
    static constexpr std::size_t const sign_bit = static_cast<std::size_t>(Q::is_signed);
    ...
    struct storage_type_promotion_traits
    {
        ...
        using type =
            typename std::conditional<
                Q::is_signed,
                typename boost::int_t <(n + delta_n) + (f + delta_f) + This_class::sign_bit>::least,
...
public:
    static constexpr bool const is_expandable =
        ((n + delta_n) + (f + delta_f) <= std::numeric_limits<max_type>::digits);
