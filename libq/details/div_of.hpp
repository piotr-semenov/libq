...
/**
    ...
    @note Consider two numbers @f$ x @f$ of format @f$ (n_x, f_x, e_x) @f$ and
    @f$ y @f$ of format @f$ (n_y, f_y, e_y) @f$.
    Let me compute the dynamic range of the quotient
    @f$ z=\frac{x}{y} @f$
    of format
    @f$ (n_z, f_z, e_z) @f$:
    - the maximum value is
    @f$
    \frac{2^{n_x} \cdot 2^{-e_x}}{2^{-f_y} \cdot 2^{-e_y}} =
    2^{n_x+f_y} \cdot 2^{-(e_x-e_y)} = 2^{n_z} \cdot 2^{-e_z}
    @f$.
    - the minimum value is
    @f$
    \frac{2^{-f_x} \cdot 2^{-e_x}}{2^{n_y} \cdot 2^{-e_y}} =
    2^{-(f_x+n_y)} \cdot 2^{-(e_x-e_y)}  = 2^{-f_z} \cdot 2^{-e_z}
    @f$.
...
#endif  // INC_LIBQ_DETAILS_DIV_OF_HPP_
