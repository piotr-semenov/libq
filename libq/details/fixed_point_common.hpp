...
/** @brief Implements the fixed-point number arithmetics.
    ...
    @note Note, this extends the Q-formats like UQn.f, Qn.f with fixed pre-scaling factor \f$2^e\f$.
    ...
    @remark Note, the supremum of \f$(n + f)\f$ is std::numeric_limits<std::uintmax_t>::digits
    in case of the unsigned numbers and
    std::numeric_limits<std::intmax_t>::digits in case of the signed numbers.
...
@endcode
    ...
    @note Please, see http://en.wikipedia.org/wiki/Q_(number_format) for details.
*/
template<
    typename value_type,
    std::size_t n,
...
public:
    ...
    /// @brief Queried number of bits to represent the fractional part of fixed-point number.
    ...
    static constexpr std::size_t const is_signed = !!std::numeric_limits<storage_type>::is_signed;
        ...
        scaling_factor()
    {
        static double const factor =
            details::exp2(-static_cast<double>(This_class::scaling_factor_exponent));

        return factor;
    }


    static_assert(This_class::number_of_significant_bits <= std::numeric_limits<largest_type>::digits,
                  "too big word size is required");

#define EXP2N(N) (std::uintmax_t(1u) << (N))

    /// @brief Scale factor for this fixed - point number.
    static constexpr std::uintmax_t const scale = EXP2N(This_class::bits_for_fractional);

    /// @brief Binary mask to extract the integral bits only from the stored integer.
    ...
    static constexpr std::uintmax_t const integer_bits_mask =
        ...
        2u * (EXP2N(This_class::bits_for_fractional + This_class::bits_for_integral - 1u) -
        ((This_class::bits_for_fractional > 0u) ? EXP2N(This_class::bits_for_fractional - 1u) : 0u)) :
        0u;
    /// @brief Binary mask to extract the fractional bits from the stored integer.
    static constexpr std::uintmax_t const fractional_bits_mask = (This_class::bits_for_fractional > 0u) ?
...
#undef EXP2N
    ...
    static std::intmax_t const least_stored_integer =
        This_class::is_signed * (-static_cast<std::intmax_t>(This_class::largest_stored_integer) - 1);
    ...
    */
        ...
        wrap(T const& _val)
    {
        ...
        if (
            _val < 0 && _val < This_class::least_stored_integer ||
    ...
    /// @brief Normalizes the input fixed-point number to be accepted by current format.
    template<
        typename T1,
        std::size_t n1,
        std::size_t f1,
    ...
    >
        ...
        : m_value(This_class::normalize(_x,
    ...
    COPY_CTR_EXPLICIT_SPECIFIER fixed_point(T const& _value)
        : m_value(This_class::calc_stored_integer_from(_value, std::integral_constant<bool, std::is_floating_point<T>::value>{}))
    {}
        ...
        operator =(fixed_point<T1, n1, f1, e1, op1, up1> const& _x)
    {
        using status_type =
            ...
            (
                ...
                static_cast<int>(This_class::bits_for_fractional) + This_class::scaling_factor_exponent
            )>;
        return
...
#undef COMPARISON_OPERATOR
    ...
    */
    ...
    /** @brief Calculates the sum of the current fixed-point number and some numeric object.
        @note If no extra significant bits is available for the promoted type then
    ...
    */
    template<
        ...
        This_class,
        libq::fixed_point<T1, n1, f1, e1, op1, up1>
        >::promoted_type
    {
        using operand_type =
    ...
    */
        ...
        typename libq::details::div_of<This_class, libq::fixed_point<T1, n1, f1, e1, Ps...> >::promoted_type
    {
        ...
        if (
            !promotion_traits::is_expandable && _x.value() !=
...
private:
    ...
    */
        ...
        calc_stored_integer_from(T const& _x, std::true_type)
    {
        ...
        double const value =
            static_cast<double>(_x) / details::exp2(scale);

        if (_x > T(0)) {
            storage_type const converted =
                static_cast<storage_type>(std::floor(value * This_class::scale + 0.5));
        ...
        return
            static_cast<storage_type>(std::ceil(value * This_class::scale - 0.5));
    }
    ...
    */
    template<typename T1, std::size_t n1, std::size_t f1, int e1, class... Ps>
    static storage_type
        normalize(fixed_point<T1, n1, f1, e1, Ps...> const& _x,
                  std::false_type)
    {
...
/// @brief Short-cut for the signed fixed-point with just 2 template parameters n and f.
template<std::size_t n, std::size_t f, int e = 0, class op = libq::ignorance_policy, class up = libq::ignorance_policy>
using Q =
