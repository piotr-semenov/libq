...
mean(std::vector<Value_type> const& _samples)
{
    ...
    Value_type const acc =
        std::accumulate(_samples.cbegin(),
                        _samples.cend(),
                        Value_type(0.0));
...
standard_deviation(std::vector<Value_type> const& _samples)
{
    ...
    Value_type moment_2 =
        std::inner_product(_samples.cbegin(),
                           _samples.cend(),
                           _samples.cbegin(),
                           Value_type(0.0));
    ...
    return
        std::fabs(moment_2 - m * m) < TOLERANCE ? Value_type(0.0) :
        Value_type{std::sqrt(moment_2 - m * m)};
}
...
test()
{
    ...
    std::cout <<
        ...
        ", " <<
        "standard_deviation=" << static_cast<double>(sd) <<
        std::endl;
}
